...
## 2026-06-25 - Avoid redundant JSON serialization on API Cache Hits
**Learning:** Returning a Python dictionary from an LRU-cached helper function still requires FastAPI's `JSONResponse` (or standard serialization) to traverse and serialize the dictionary into JSON on every cache hit. For endpoints returning large primitive arrays (like 1000+ point trajectories), this repeated JSON stringification creates measurable CPU overhead and allocations, even when the mathematical computation itself is successfully cached.
**Action:** When caching API endpoints that return large payloads, always serialize the result to JSON string or bytes *before* returning from the cached helper function (e.g., `return json.dumps(res).encode('utf-8')`). In the endpoint, return this cached payload directly using FastAPI's base `Response(content=..., media_type="application/json")`. This completely eliminates JSON serialization overhead on cache hits, yielding true O(1) response times.
...
## 2026-08-10 - Minify API JSON Payload Size
**Learning:** When manually serializing large JSON payloads (like multi-dimensional arrays or trajectories) in Python, default `json.dumps()` includes unnecessary whitespace which increases payload size.
**Action:** When manually serializing large JSON payloads (like multi-dimensional arrays or trajectories) in Python, use `json.dumps(res, separators=(',', ':'))` to minify the output. Stripping unnecessary whitespace reduces payload size by ~15-20%, which improves memory allocation efficiency and network transfer speeds.
