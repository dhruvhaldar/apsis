...
## 2024-05-24 - Minify Large JSON Serialization
**Learning:** When manually serializing large JSON payloads (like multi-dimensional arrays or trajectories) in Python, default json.dumps includes unnecessary whitespace. Stripping this whitespace reduces payload size by ~15-20%, which improves memory allocation efficiency and network transfer speeds.
**Action:** Use json.dumps(res, separators=(',', ':')) to minify the output.
