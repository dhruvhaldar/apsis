...
## 2024-10-24 - Minify JSON manually
**Learning:** When manually serializing large JSON payloads (like multi-dimensional arrays or trajectories) in Python, use `json.dumps(res, separators=(',', ':'))` to minify the output. Stripping unnecessary whitespace reduces payload size by ~15-20%, which improves memory allocation efficiency and network transfer speeds.
**Action:** Always include `separators=(',', ':')` in `json.dumps()` for large arrays to improve performance.
