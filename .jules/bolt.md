...
## 2024-08-20 - JSON Payload Minification
**Learning:** Stripping unnecessary whitespace from large JSON payloads (like multi-dimensional arrays or trajectories) using `json.dumps(res, separators=(',', ':'))` reduces payload size by ~15-20%, which improves memory allocation efficiency and network transfer speeds.
**Action:** When manually serializing large JSON payloads in Python, use `json.dumps(res, separators=(',', ':'))` to minify the output.
