...
## $(date +%Y-%m-%d) - Scope DOM-Traversing Libraries
**Learning:** Initializing DOM-traversing parsing libraries (like KaTeX) globally using `renderMathInElement(document.body)` forces the library to perform a deep recursive traversal of the entire DOM tree on page load. It scans every text node, traversing massive unrelated sub-trees like Three.js canvas containers, complex forms, and SVGs. This creates severe initialization overhead and blocks the main thread, delaying Time to Interactive.
**Action:** Always scope DOM-traversing parser initialization explicitly to the containers where parsing is actually needed (e.g., `document.querySelectorAll('.math-block').forEach(...)`). This limits tree traversal strictly to the relevant nodes, drastically speeding up initial page loads.
