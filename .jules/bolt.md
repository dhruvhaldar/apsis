...
## 2026-05-15 - Conditional DOM Attribute Writes on High-Frequency Events
**Learning:** Unconditionally writing to DOM attributes (like `setAttribute` or `setCustomValidity`) inside high-frequency event listeners (like `input` or `mousemove`) forces the browser to synchronously invalidate and recalculate styles on every single trigger, even if the written value is identical to the current one. This severely delays the main thread and causes UI lag (poor Interaction to Next Paint).
**Action:** Always check the current state of a DOM attribute or property before writing to it in high-frequency event listeners. Wrap DOM writes in a conditional check (e.g., `if (el.getAttribute('attr') !== 'value')`) to short-circuit redundant updates and preserve main thread performance.
