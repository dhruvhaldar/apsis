...
## 2026-06-27 - Prevent Disabled Styles on Temporary Success States
**Learning:** Applying generic disabled styling (like `opacity: 0.5` and `cursor: not-allowed`) to interactive elements during short-lived success states (e.g., a "Copied ✅" button) creates a confusing UX. The user expects positive reinforcement, but the disabled visual cues convey an error or "unavailable" state, even if the element is only temporarily ignoring input to prevent spam clicks.
**Action:** Always decouple short-lived success state styling from generic disabled styling. When temporarily disabling interactions for success feedback (e.g., via `data-copying="true"`), explicitly use `opacity: 1`, `cursor: default`, and positive success colors (like green/cyan) rather than inheriting visually negative disabled attributes.
...
## 2026-07-04 - Proactive Screen Reader Announcements for Inline Validation
**Learning:** Sighted users receive immediate visual feedback (e.g., red borders, error icons) when they type invalid data and step out of an input field (`focusout`). However, without explicitly triggering an `aria-live` announcement, screen reader users only hear this validation error later if they explicitly re-focus the field or attempt to submit the form. This creates an unequal, delayed feedback loop for non-visual users.
**Action:** When implementing inline validation logic that triggers on events like `focusout`, always use an `aria-live` announcer to proactively read out the specific validation error message (including the input's label for context). This ensures screen reader users receive immediate, contextual feedback identical to the visual cues provided to sighted users.

## 2026-07-06 - Explicitly Linking Form Submit Buttons to Output Regions
**Learning:** When a form updates a specific, decoupled output region (like a chart container or a results block) rather than causing a full page reload, screen reader users might not immediately realize which part of the page will be affected by their submission. While the results might be correctly announced via `aria-live` once generated, the spatial relationship between the trigger and the output is opaque.
**Action:** Always add the `aria-controls` attribute to form submit buttons, setting its value to the ID of the output container they update (e.g., `aria-controls="pmp-chart"`). This provides screen readers with explicit structural linkage, allowing users to understand the interface's cause-and-effect relationship before they interact. Ensure the referenced ID encompasses all possible states (empty/output) of that region.

## 2026-07-06 - Accessible CSS Loading Spinners and Reduced Motion
**Learning:** Continuous CSS loading animations (like spinners or pulsing dots) are visually appealing and convey system state effectively for most users. However, for users with vestibular motion disorders (or those who simply prefer reduced motion), continuous non-essential animations can be distracting or nauseating. If a loading state relies entirely on a CSS `animation` (e.g., `transform: rotate`), disabling that animation via `@media (prefers-reduced-motion: reduce)` without providing a visual fallback leaves the UI looking broken (e.g., a static, partially-drawn circle).
**Action:** When implementing CSS loading animations (like a `.spinner` class), always pair the `@media (prefers-reduced-motion: reduce)` block with a static visual fallback. For example, explicitly set `animation: none !important;` and change the border style (e.g., `border-style: dotted !important;`) so the spinner still visually indicates a "loading" state without requiring continuous rotation.
...
## 2026-07-29 - Confirm Destructive Actions
**Learning:** Adding a "Reset" button to clear complex mathematical forms provides a fast way for users to start over. However, because these forms take significant time to fill out accurately, accidentally clicking "Reset" (which might sit near a primary action button or heading) results in catastrophic data loss and frustration.
**Action:** Always intercept destructive actions like form resets with a confirmation dialog (e.g., `window.confirm('Are you sure you want to reset this form? All entered data will be lost.')`). This introduces a necessary moment of friction to prevent accidental data destruction while keeping the feature available for intentional use.

## 2026-07-30 - Primary Button Text Contrast with Bright Hover States
**Learning:** Using white text (`#fff`) on a dark-ish button background might barely pass contrast requirements initially, but if the button's `:hover` state brightens the background significantly (e.g. transitioning to a bright cyan like `#66fcf1`), the contrast ratio collapses (e.g. down to 1.1:1). This creates a severe accessibility failure where the primary call-to-action text becomes virtually invisible upon interaction, especially for users with low vision.
...
## 2024-11-20 - Clear Output on Form Resets
**Learning:** Adding a "Reset" button to clear complex mathematical forms provides a fast way for users to start over. However, resetting only the form inputs leaves old outputs (like charts and synthesized control values) visible on the screen. This creates a confusing disconnect where the form inputs have returned to defaults but the resulting output still displays the previous computation.
**Action:** When handling form resets, always explicitly clear the corresponding output containers and restore them to their initial "empty state" (with appropriate icons/messages) to provide the user with a true clean slate that reflects the reset input values. If using chart libraries (like Chart.js), remember to properly `.destroy()` the instances to avoid memory leaks or rendering conflicts when the user submits new data.
