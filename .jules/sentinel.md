...
## 2026-05-18 - Validation Error JSON Serialization Denial of Service (DoS)
**Vulnerability:** A Denial of Service (DoS) vulnerability via server crash could occur due to how FastAPI and Pydantic handle out-of-range float values (like `Infinity` or `NaN`). While Pydantic correctly invalidates these inputs, the resulting `RequestValidationError` includes the raw invalid value in the `input` field of its detail structure. When FastAPI attempts to serialize this error response using `JSONResponse` (which internally uses `json.dumps`), it throws a `ValueError: Out of range float values are not JSON compliant: inf`. This unhandled exception crashes the process/thread and returns a 500 Internal Server Error, allowing an attacker to continually crash the server by repeatedly submitting payloads with `Infinity`.
**Learning:** `JSONResponse` in FastAPI does not safely serialize `Infinity` or `NaN` by default because they are not strictly valid JSON. If an attacker's raw invalid input is blindly reflected back in the error response, it can cause secondary serialization crashes.
**Prevention:** Always sanitize or remove the raw `input` field from `RequestValidationError` structures before returning them to the user. Implement a custom `@app.exception_handler(RequestValidationError)` that loops through `exc.errors()`, safely deletes the `input` key from each error dictionary, and then returns the sanitized errors via `JSONResponse`.

## 2024-05-28 - Enhance Rate Limiting Privacy & Security Headers
**Vulnerability:** IP addresses (PII) were stored in plaintext in the in-memory rate limit store, and API responses lacked caching prevention headers.
**Learning:** Storing IPs in plaintext risks exposing PII if memory is dumped. Missing cache headers can cause sensitive data to be stored by proxies.
**Prevention:** To prevent the exposure of Personally Identifiable Information (PII) in the event of a memory dump or compromise, always hash client IP addresses (e.g., using `hashlib.sha256`) before storing them in memory for tracking or rate limiting. Ensure sensitive API responses include the `Cache-Control: no-store, no-cache, must-revalidate, max-age=0` header to prevent intermediate proxies and client browsers from inadvertently caching the data.

## 2026-05-18 - Rate Limiter Algorithmic Complexity DoS
**Vulnerability:** The rate limiter performed an O(N) cleanup iteration over the entire `rate_limit_store` dictionary (filtering out old timestamps for every IP) on every single request whenever the dictionary size exceeded `MAX_IPS`. An attacker could spoof IPs to hit the `MAX_IPS` limit, and then send a flood of requests. Each request would trigger the slow O(N) loop synchronously, blocking the FastAPI event loop and causing an Algorithmic Complexity Denial of Service (DoS) for all users.
**Learning:** Performing O(N) operations in the critical path of a web request handler, especially without `await` inside an `async def` function, will synchronously block the event loop and lead to DoS under load.
**Prevention:** Only run heavy cleanup or eviction logic when absolutely necessary (e.g., `if len(rate_limit_store) >= MAX_IPS and hashed_ip not in rate_limit_store:`). Optimize the eviction check itself (e.g., checking if the last timestamp in a list is older than the window, rather than reconstructing the list of timestamps on every check).

## 2025-05-19 - Robust Resource Cleanup in `try...finally` Blocks
**Vulnerability:** A `try...finally` block intended to clean up resources (like temporary directories used by GEKKO) left the resource instantiation vulnerable to exceptions. If an exception occurred immediately *after* the `GEKKO` class instantiation but *before* the `try` block started, the `m` object would be created without any guarantee that `finally: m.cleanup()` would execute, leading to disk resource exhaustion.
**Learning:** For robust resource management, the reference variable must be initialized before the `try` block (e.g., `m = None`), and the resource instantiation itself *must* occur inside the `try` block.
**Prevention:** Always follow the pattern: initialize to `None`, open/instantiate inside `try`, and check `if resource is not None:` in `finally` before cleaning up. This prevents `UnboundLocalError` if the instantiation fails, and ensures proper cleanup if an asynchronous exception or signal arrives immediately post-instantiation.
## 2024-06-03 - In-Memory PII Rainbow Table Vulnerability
...
## 2026-07-03 - Bypass of Middleware Route Matching
**Vulnerability:** A path manipulation vulnerability allowed attackers to bypass the ASGI-level rate limiter by inserting extra slashes or URL-encoded characters (e.g., `//api/lqr` or `/%2fapi/lqr`). Since `request.scope.get('path')` returns the raw, unnormalized path, the `.startswith('/api/')` check failed, yet FastAPI's underlying router still successfully routed the request to the expensive endpoints.
**Learning:** Checking raw ASGI scope paths (`request.scope.get('path')`) for security enforcement (like auth or rate limiting) is dangerous because it lacks the path normalization that the web framework performs downstream.
**Prevention:** Always normalize the path by unquoting and collapsing duplicate slashes (e.g., `re.sub(r'/+', '/', urllib.parse.unquote(path))`) before applying substring-based security checks in middleware.
## 2026-07-13 - Rate Limiter IP Spoofing Global DoS
**Vulnerability:** The rate limiter blindly trusted the `X-Forwarded-For` header. When the `rate_limit_store` reached its maximum capacity (`MAX_IPS = 10000`) and the IP was not already tracked, it would attempt to clean up stale IPs. If the store was filled entirely with recent requests from an attacker utilizing IP spoofing (or a botnet), the cleanup would fail to evict any IPs. The middleware would then universally return a `503 Service temporarily unavailable` response to *any* new IP. This allowed attackers to easily orchestrate a persistent, global Denial of Service for all new legitimate users simply by rotating spoofed IP addresses and keeping the store artificially filled with recent timestamps.
**Learning:** Returning a global error (like 503) when a bounded state mechanism (like an in-memory dictionary) fills up creates a trivial vector for DoS via resource exhaustion, especially when the key to that state (like client IP) can be trivially spoofed or generated.
**Prevention:** Instead of locking out the entire service when a rate limiting cache fills up, prioritize legitimate users by employing an eviction strategy (like LRU - Least Recently Used). If the cache is full and no IPs are naturally stale, evict the oldest inserted IP (e.g., using `next(iter(store))`) to make room for the new incoming request. This ensures the service remains available and gracefully degrades cache hit rates rather than completely denying service.
